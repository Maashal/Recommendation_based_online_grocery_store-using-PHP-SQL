...
$id=$_GET['id'];
  ...
  $run= mysqli_query($con, "INSERT INTO `reject_purchase_request`(`p_id`, `customer_id`, `quantity`, `total_amount`) VALUES ('$p_id','$customer_id','$quantity','$total_amount')");
  if($run)
  {
      $run1= mysqli_query($con, "DELETE FROM `purchase_request` WHERE id='$id'");
      if($run1)
      {      
         alert("Sucessfully reject Purchase Request!");
